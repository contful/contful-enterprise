...
type StorageConfigFunc func(ctx context.Context, siteID uid.UID) (*ProviderConfig, string, error)
...
func (m *StorageManager) ProviderFor(ctx context.Context, siteID uid.UID) (StorageProvider, error) {
	return m.provider, nil
}

// Invalidate 不再需要（全局模式配置变更需重启服务）
func (m *StorageManager) Invalidate(siteID uid.UID) {}

// InvalidateAll 不再需要（全局模式配置变更需重启服务）
func (m *StorageManager) InvalidateAll() {}

// defaultStorageConfig 默认配置（fallback）
func defaultStorageConfig(_ context.Context, _ uid.UID) (*ProviderConfig, string, error) {
