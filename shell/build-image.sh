...
# 自动检测当前平台架构，只负责架构判断和标签管理
# 实际构建逻辑（含前端）全部在 Dockerfile 中完成
# =============================================================================
#
# 用法:
...
build_console_image() {
    ...
    log_info "  标签: contful/console:${tag_arch} + contful/console:${tag_latest}"
    ...
    log_success "Console 镜像构建完成: contful/console:${tag_arch} + :${tag_latest}"
    echo ""
}
...
build_openapi_image() {
    ...
    log_info "  标签: contful/openapi:${tag_arch} + contful/openapi:${tag_latest}"
    log_info "  数据库: ${db_type}, 平台: ${platform}"
    ...
    log_success "Open API 镜像构建完成: contful/openapi:${tag_arch} + :${tag_latest}"
    echo ""
}

# 构建所有 Console（所有 DB_TYPE）
build_console() {
    local arch=$(get_arch)
    local platform=$(get_platform)
    [ -z "$platform" ] && { log_error "不支持的架构: $(uname -m)"; exit 1; }
...
build_all() {
    ...
    log_info "当前架构: $(uname -m) -> $platform"
    echo ""
